...
*	Description: This class has been modified to include a method getACycle which will detect whether or not a cycle exists in this Graph. The method will return a List
*	containin the first instace of a cycle, otherwise it returns null.
*/
import java.util.*;
...
public class UnweightedGraph<V> implements Graph<V> {
  ...
  //This method returns a List containing a cycle inside of this Graph. If no cycles exist it returns null.
  public List<Integer> getACycle(){
	//Initialize a list containing the numerical index of each vertex.
	List<Integer> allVertices = new ArrayList<Integer>();
	for(int i = 0; i < vertices.size(); i++){
		allVertices.add(i);
	}
	//Create a boolean array to determine if a vertex has been visited.
	boolean[] isVisited = new boolean[vertices.size()];
	
	//Create an int array to determine the parent of a vertex.
	int[] parent = new int[vertices.size()];
	
	while(allVertices.size() > 0){
		//We pick the first vertex which has not been visited.
		int v = allVertices.get(0);
		Stack<Integer> stack = new Stack<Integer>();
		stack.push(v);
		isVisited[v] = true;
		allVertices.remove(new Integer(v));
		//If the stack is not empty we look at the top vertex an determine if it has any neighbors.
		while(!stack.isEmpty()){
			int x = stack.peek();
			if(neighbors.get(x).size() == 0){
				...
				for(int i = 0; i < neighbors.get(x).size(); i++){
					...
					//If the next vertex has not been visited we add it to the stack.
					if(!isVisited[e.v]){
						parent[e.v] = x;
						stack.push(e.v);
						isVisited[e.v] = true;
					...
					//If the parent of x is not one of its neighbors then we have not found a cycle yet.
					else if(parent[x] != e.v){
						ArrayList<Integer> list = new ArrayList<Integer>();
						list.add(e.v);
						//Here we loop to build a potential cycle.
						while(x != e.v && x != -1){
							list.add(x);
							x = parent[x];
						}
